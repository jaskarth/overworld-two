...
@Mixin(CountBiasedRangeDecorator.class)
public class MixinCountBiasedRangeDecorator {
    private final DecoratorStream<RangeDecoratorConfig> stream = new CountRangeStream((config, random) -> random.nextInt(random.nextInt(config.maximum - config.topOffset) + config.bottomOffset));
