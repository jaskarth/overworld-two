...
@Mixin(CountVeryBiasedRangeDecorator.class)
public class MixinCountVeryBiasedRangeDecorator {
    private final DecoratorStream<RangeDecoratorConfig> stream = new CountRangeStream((config, random) -> random.nextInt(random.nextInt(random.nextInt(config.maximum - config.topOffset) + config.bottomOffset) + config.bottomOffset));
